...
<#
	...
	.NOTES
		...
		28.02.2013 MS: Check vDisk conversation successfully, then shutdown
		...
		19.09.2014 MS: Get content from log files and write to log file
		20.08.2014 MS: Removed log content from P2PVS, because XenConvert has a size of 100 MB and more
		15.09.2014 MS: P2PVS.log or XenConvert.log would be checked if PVS Target Device is installed and the Device boot up from hard drive only
		...
		14.04.2015 MS: Prevented shutdown the Base Image after successful conversion of the image, this switch would be automatically set if the Base Image preparation is running from SCCM/MDT Task Sequence
		14.04.2015 MS: Get-vDiskDriveLetter to defrag vDisk after successful build, no longer defrag the hard drive
		14.04.2015 MS: Rewritten POST Build Action $PostCommands
		...
		28.10.2016 MS: Bug fix Sysprep;  Sysprep is not running in earlier BIS-F Version. Adding error handling, checking setuperr.log for errors and perform postCommands
		...
		29.10.2016 MS: After successful sysprep, shutdown computer would be performed only, if not suppressed by CLI command
		...
		20.04.2017 MS: Issue 175: - After patch tuesday in April 2017 powershell command stop-computer does not work as expected (privilege not held), using shutdown /s now
		01.05.2017 MS: Fixed: after successful sysprep, running PostCommand now for defrag and shutdown
		01.05.2017 MS: Fixed 178: defrag arguments are different between client and server os, thx to Jeremy Saunders
		31.07.2017 MS: Fixed: if Citrix PVS Target Device Driver and Citrix AppLayering is installed, PostCommand would not executed
		02.08.2017 MS: IF ADMX for custom VHDX UNC-Path is enabled, Defrag can't performed
		06.08.2017 MS: from every P2V conversion, the log file would be included into the BIS-F log, instead of error only
		22.08.2017 MS: If defrag not run, write-out the DiskMode to the BIS-F log for further analysis if possible to run
		24.08.2017 MS: If AppLayering is installed and running not inside ELM, the VM is build first time, run defrag on system drive
		11.09.2017 MS: Writing PersSate "NotRunning" to BISF Registry to control running prep after pers first
		29.10.2017 MS: Fixed: defrag select the the right vDisk on the custom UNC-Path or the direct conversion
		29.10.2017 MS: Fixed: IF $DiskNameExtension -eq "noVirtualDisk" and custom UNC-Path is enabled, running OfflineDefrag on custom UNC-Path
		01.11.2017 MS: check if Defrag Service is running, thx to Lejkin Dmitrij
		02.11.2017 MS: Fixed: if booting up in private Mode the vhdx and custom unc-path is configured, defrag runs on the UNC-Path and not on the BaseDisk itself.
		...
		20.12.2019 MS/SF: FRQ 154 (PR)- Adjust for compositing engine change in AppLayering 1911 an higher
		...
		28.06.2020 MS: HF 257 - Azure WVD not detected -> Update Platform info, if no Image Management is detected
		16.08.2026 JP: Default Sysprep arguments include /mode:vm on VM guests (IsVirtualMachine); omit on physical/unknown
...
Begin {
	...
	IF ($DefragArgs -eq "") {
		$DefragArgs = "/U"
		Write-BISFLog -Msg "Unsupported OS-Version $OSVersion detected. The defrag arguments will be set to standard values: $DefragArgs" -Type W
	}
	ELSE {
	...
	IF ($LIC_BISF_CLI_SB -eq "") {
		$LIC_BISF_CLI_SB = "YES"
		Write-BISFLog -Msg "CLI Switch for Shutdown Image (-Shutdown YES or -Shutdown NO) not specified, it will be set to shutdown the Image after successful build"
	}
	...
	$PostCommands += [PSCustomObject]@{Order = "998"; Enabled = "$true"; showmessage = "N"; CLI = ""; Description = "Set Personalization State in Registry, to control Preparation is running after Personalization first"; Command = "Set-ItemProperty -Path '$HklmBisfScripts' -Name 'LIC_BISF_PersState' -Value '$PersState' -force " }
	...
	####################################################################
	# Post Command after successful build vDisk
	...
	function Start-AppLayeringLayerFinalize {
		IF (!($CTXAppLayerName -eq "No-ELM")) {
			IF ($CTXAppLayeringSW) {
				...
				ELSE {
					Write-BISFLog -Msg "Layer finalize is NOT allowed, this issue is sending out from AppLayering and not BIS-F, please check the BIS-F log for further information" -SubMsg -Type E
				}
...
Process {
	...
	#### Main Program
	Write-BISFLog -Msg "Write Sysprep status to registry location Path: $HklmBisfScripts -Name: LIC_BISF_RunSysPrep -Value: $RunSysPrep"
	Set-ItemProperty -Path $HklmBisfScripts -Name "LIC_BISF_RunSysPrep" -Value "$RunSysPrep" #-ErrorAction SilentlyContinue

	Start-AppLayeringLayerFinalize
	...
	IF ($ReturnTestPVSSoftware -eq $true) {
		IF ($CTXAppLayeringSW) {
			...
			if (Test-Path $BootIdPath) {
				...
				$ShutdownFlag = "/r"
				$PostCommands = $PostCommands | Where-Object { $_.order -ne "999" }
		...
		ELSE {
			IF ($SkipPVSImaging -eq $false) {
				IF ($CheckP2PVSlog -eq $true) {
					$CheckPVSLog = Test-BISFLog -CheckLogFile "$P2PVSLogFile" -SearchString "$P2PVSLogFileSearch"
					get-BISFLogContent -GetLogFile "$P2PVSLogFile"
					IF ($CheckPVSLog -ne "") {
						Write-BISFLog -Msg "Successfully built your Image..." -ShowConsole -Color DarkCyan -SubMsg
						Write-BISFLog -Msg "vDisk $P2PVSLogFileSearch"
						PostCommand
					}
					ELSE {
						Write-BISFLog -Msg "vDisk operation NOT successful, check $P2PVSLogFile for further details" -Type E
					}
				}
				...
				IF ($CheckP2PVSlog -eq "ERROR") {
					get-BISFLogContent -GetLogFile "$P2PVSLogFile"
					Write-BISFLog -Msg "vDisk operation NOT successful, check $LIC_PVS_LogPath for further details" -Type E
				}
			}
			ELSE {
	...
	ELSE {
		...
		ELSE {
			IF ($RunSysPrep -eq $true) {
				...
				Write-BISFLog -Msg "Write Sysprep ServiceList to registry location: $HklmBisfScripts -Name: LIC_BISF_SysPrep_ServiceList -Value: $LIC_BIS_Sysprep_ServiceList"
				Set-ItemProperty -Path $HklmBisfScripts -Name "LIC_BIS_Sysprep_ServiceList" -Value "$LIC_BIS_Sysprep_ServiceList" #-ErrorAction SilentlyContinue
				...
				ELSEIF ($IsVirtualMachine) {
					$args = "/generalize /oobe /mode:vm /quiet /quit"
					Write-BISFLog -Msg "VM guest detected ($ReturnGetHypervisor); using /mode:vm" -ShowConsole -Color DarkCyan -SubMsg
				}
				ELSE {
					$args = "/generalize /oobe /quiet /quit"
					Write-BISFLog -Msg "Physical or unknown platform ($ReturnGetHypervisor); omitting /mode:vm" -ShowConsole -Color DarkCyan -SubMsg
				}
			...
			ELSE {
				Write-BISFLog -Msg "No Image Management Software detected [Citrix VDA, Citrix PVS Target Device Driver, Citrix App Layering, Omnissa Horizon Agent, Parallels RAS Agent, Dizzion Frame, Azure AVD]" -Type W
				Write-BISFLog -Msg "The system will not be shut down by this script. Run Sysprep or your preferred method manually, or install one of the products above and run the script again" -Type W
				Start-Sleep -s 30
			}
		}
	}
