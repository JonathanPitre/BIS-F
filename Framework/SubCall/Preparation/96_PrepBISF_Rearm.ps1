...
<#
	...
	.NOTES
		...
		18.09.2013 MS: Replaced $date with $(Get-date) to get current timestamp at running script lines write to log file
		18.09.2013 MS: Set Office rearm status to 0, if office x86 not detected
		...
		13.03.2013 MS: Remove function MigrateValues > add message box to OS rearm and office rearm
		18.03.2014 BR: Revisited Script
		13.05.2014 MS: Added silent switch -OSrearm (YES|NO)-OFrearm (YES|NO)
		...
		16.10.2017 MS: Fixed OS rearm never run, path to slmgr.vbs must be entered before, thx to Bernd Baedermann
		17.10.2017 MS: Fixed Running Office rearm first and second OS rearm, thx to Bernd Baedermann
		...
		09.11.2017 MS: Read Office installation path from registry, support now for Office x64 and x86
...
Begin {
	...
	$RearmRegName1 = "LIC_BISF_RearmOS_run"
	$RearmRegName2 = "LIC_BISF_RearmOS_user"
	$RearmRegName3 = "LIC_BISF_RearmOS_date"
	$RearmRegName4 = "LIC_BISF_RearmOF_run"
	$RearmRegName5 = "LIC_BISF_RearmOF_user"
	$RearmRegName6 = "LIC_BISF_RearmOF_date"
	...
	ForEach ($OfficeProduct in $OfficeProducts) {
		...
		IF ($null -ne $Office) {
			...
			Write-BISFLog -Msg "Install path $OfficeInstallRoot " -ShowConsole -Color DarkCyan -SubMsg
	...
	function RearmOS {
		...
		Write-BISFLog -Msg "check OS rearm registry keys in $HklmBisfScripts"
		...
		IF ($LIC_BISF_RearmOS_run -ne 1) {
			...
			if (($OSReArmAnswer -eq "YES" ) -or ($VarCLI -eq "YES")) {
				...
				Start-BISFProcWithProgBar -ProcPath "$env:windir\system32\cscript.exe" -Args "//NoLogo $env:windir\system32\slmgr.vbs /dlv" -ActText "OS - Get detailed license information"
			}
			Write-BISFLog -Msg "Set specified registry keys in $HklmBisfScripts"

			Write-BISFLog -Msg "Set Value = $RearmRegName1 / ValueData = 1"
			Set-ItemProperty -Path $HklmBisfScripts -Name $RearmRegName1 -Value "1" #-ErrorAction SilentlyContinue

			Write-BISFLog -Msg "Set Value = $RearmRegName2 / ValueData = $CurrentUser"
			Set-ItemProperty -Path $HklmBisfScripts -Name $RearmRegName2 -Value $CurrentUser #-ErrorAction SilentlyContinue

			Write-BISFLog -Msg "Set Value = $RearmRegName3 / ValueData = $(Get-Date)"
			Set-ItemProperty -Path $HklmBisfScripts -Name $RearmRegName3 -Value $(Get-Date) #-ErrorAction SilentlyContinue

		}
		ELSE {
			Write-BISFLog -Msg "Operating System already rearmed, no action needed" -ShowConsole -Color DarkCyan -SubMsg
			Start-BISFProcWithProgBar -ProcPath "cscript.exe" -Args "//NoLogo $env:windir\system32\slmgr.vbs /dlv" -ActText "Get detailed OS license information"
		}
	}
	####################################################################
	...
	function RearmOffice {
		...
		IF ($O365onAzure -eq $true) {
			...
			Start-BISFProcWithProgBar -ProcPath "$env:windir\system32\dsregcmd.exe" -Args "/leave" -ActText "Office - Performs Hybrid Join leave"
		...
		IF ($null -ne $OSPPREARM) {
			IF (Test-Path -Path $OSPPREARM) {
				$OSPPrearmPath = [System.IO.Path]::GetDirectoryName($OSPPREARM)
				Write-BISFLog -Msg "Office detected for rearm, check $OSPPREARM"
				Write-BISFLog -Msg "Check Office rearm registry keys in $HklmBisfScripts"
				...
				IF ($LIC_BISF_RearmOF_run -ne 1) {
					...
					if (($OFReArmAnswer -eq "YES" ) -or ($VarCLI -eq "YES")) {
						...
						Start-BISFProcWithProgBar -ProcPath "$env:windir\system32\cscript.exe" -Args "//NoLogo ""$OSPP"" /dstatus" -ActText "Office - Get detailed license information after rearm"

					}
					Write-BISFLog -Msg "Set specified registry keys in $HklmBisfScripts"
					Write-BISFLog -Msg "Set Value = $RearmRegName4 / ValueData = 1"
					Set-ItemProperty -Path $HklmBisfScripts -Name $RearmRegName4 -Value "1" #-ErrorAction SilentlyContinue
					Write-BISFLog -Msg "Set Value = $RearmRegName5 / ValueData = $CurrentUser"
					Set-ItemProperty -Path $HklmBisfScripts -Name $RearmRegName5 -Value $CurrentUser #-ErrorAction SilentlyContinue
					Write-BISFLog -Msg "Set Value = $RearmRegName6 / ValueData = $(Get-Date)"
					Set-ItemProperty -Path $HklmBisfScripts -Name $RearmRegName6 -Value $(Get-Date) #-ErrorAction SilentlyContinue
				}
				ELSE {
					Write-BISFLog -Msg "Office already rearmed, no action needed" -ShowConsole -Color DarkCyan -SubMsg
					Start-BISFProcWithProgBar -ProcPath "cscript.exe" -Args "//NoLogo ""$OSPP"" /dstatus" -ActText "Get detailed Office license information"
				}
			}
			ELSE {
				Write-BISFLog -Msg "Office for rearm not found, check $OSPPREARM"
				Write-BISFLog -Msg "Set Office rearm status to never run in $HklmBisfScripts"
				Write-BISFLog -Msg "Set Value = $RearmRegName4 / ValueData = 0"
				Set-ItemProperty -Path $HklmBisfScripts -Name $RearmRegName4 -Value "0" #-ErrorAction SilentlyContinue
			}
		}
		ELSE {
