<#
	.SYNOPSIS
		Prepare Base Image for Image Management Software like PVS, MCS, VMwareView or ask for Sysprep if nothing of them is installed
	.DESCRIPTION
		Use the parameters for CLI Switches
		For silent automation please configure BIS-F with the additional ADMX templates that can be found in your BIS-F installation folder
	.PARAMETER Verbose
		YES,NO = Enable verbose mode to show suppressed messages on console
	.PARAMETER Debug
	...
	.NOTES
		...
		03.07.2013 MS: Write script path to registry $hklm_software\PS-SCRIPTS\COMMON to use in hole environment
		26.08.2013 MS  Removed $XA_PrpPVS_Folder = $SubCall_Folder + "60_XA_PrpPVS\"
		16.09.2013 MS: Added custom folder 99_XX_Custom\20_XX_PrepPVS
		16.09.2013 MS: Load_PS_Folder -def_load_PS_Folder $LIB_Folder
		17.09.2013 MS: Remove unused variable and get folder names from LIB_Config
		18.09.2013 MS: Replace $date with $(Get-date) to get current timestamp at running script lines write to the log file
		18.09.2013 MS: Predefine $LIB_Folder & $SubCall_Folder
		19.09.2013 MS: IF ($scripts -ne $null)
		17.12.2013 MS: Added $return for error handling and exiting script
		28.01.2014 MS: Changed $return = Invoke-Expression $item.FullName to Invoke-Expression $item.FullName for error handling return code
		10.03.2014 BR/MS: Showmessage box, review Code
		10.03.2014 MS: Changed to $ScriptsFolder = $SubCall_Folder + "15_XX_Custom\20_XX_PrepPVS"
		...
		18.08.2014 MS: Added computer name to log file name $Global:LogFileName = "Prep_BIS_$($computer)_$timestamp.bis"
		15.09.2014 MS: Added CLI Switch -P2PVS to use P2PVS instead of XenConvert if installed
		15.09.2014 MS: Added CLI switch -verbose -> $LIC_PVS_CLI_VB to show suppressed messages in console
		...
		15.12.2015 MS: Added CLI option DisableIPv6 to disable IPv6 completely
		28.01.2016 MS: Added CLI option Sysprep to use them of No Image Management Software detected
		10.03.2016 MS: Added CLI switch DisableConsoleCheck to disable the check if preparation phase is running in console session
		17.03.2016 MS: Added CLI option TurboUpdate to update Turbo.net Subscription on system startup
		...
		11.01.2017 MS: Added Cli command 'RESWASdisableBaseImage' to disable RES ONE Automation Agent on Base Image only to prevent RES ONE License usage for your Base Image
		...
		12.09.2017 MS: using progress bar during wait for the personalization is finished
		16.10.2017 MS: Fixed detecting wrong POSH Version if running BIS-F remotely, using $PSVersionTable.PSVersion.Major, thx to Fabian Danner
		12.07.2018 MS: Fixed 40: PendingReboot - give a empty value back
		...
		08.10.2019 MS: ENH 146 - Removing Test for Get-PendingReboot out of this script and included into BISF.ps1 to an earlier state
		20.01.2020 MS: HF 191 - MaxExecution is not set correctly
		18.02.2020 JK: Grammar fixup
		13.08.2026 JP: Close Fluent splash after Initialize-BISFConfiguration (no 5s dead wait)
#>
...
Begin {
	...
	try {
		$WPTEnabled = (Get-ItemProperty $RegPath -Name "LIC_BISF_CLI_LOG_WPT" -ErrorAction Stop).LIC_BISF_CLI_LOG_WPT
	}
	...
	$Global:LIC_BISF_CLI_FS = @() #FS = FSLogix Delete Rules
	$Global:LIC_BISF_CLI_RS = @() #RS = FSLogix Rules Share
	$Global:LIC_BISF_CLI_SM = @() #SM = Delete All Users Start Menu
...
Process {
	#check Powershell minimum version 3.0
	$PSverMin = "3"
	...
	# 10.10.2019 Show splash screen
	# 13.08.2026 JP: Fluent dialog stays up while configuration initializes
	...
	IF ($null -eq $LIC_BISF_CLI_PersonalizationOverrideTimeOut) {
 ...
 ELSE {
	...
	DO {
		...
		if ((Get-Date) -ge $MaximumExecutionTime) {
			...
			Set-ItemProperty -Path $HklmBisfScripts -Name "LIC_BISF_PersState" -Value "$PersState" -Force
		}

		$PersState = (Get-ItemProperty $HklmBisfScripts -Name "LIC_BISF_PersState").LIC_BISF_PersState
	...
	$ScriptsFolder = $SubCall_Folder + "Preparation"
	Invoke-BISFFolderScripts -Path "$ScriptsFolder" -Verbose:$VerbosePreference
